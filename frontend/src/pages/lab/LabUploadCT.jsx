...
export default function LabUploadCT() {
  ...
  const handleSubmit = (e) => {
    ...
    if (patient) {
      ...
      alert(t('labUploadCT.successMessage', 'CT report uploaded successfully!'));
      setObservations("");
      setPatient(null);
    }
  };
  ...
  return (
    <div className="min-h-[80vh] bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('labUploadCT.title', 'Upload CT Reports')}</h1>
          <p className="text-gray-500">{t('labUploadCT.description', 'Upload and process CT scans')}</p>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">{t('labUploadCT.selectPatient', 'Select Patient')}</label>
              <div className="relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                <input
                  type="text"
                  placeholder={t('labUploadCT.searchPatient', 'Search patient by name or phone...')}
                  value={patientSearch}
              ...
              {patient && (
                <div className="mt-2 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between">
                  ...
                  <button onClick={() => setPatient(null)} className="text-sm text-emerald-700 hover:underline">{t('labUploadCT.changePatient', 'Change')}</button>
                </div>
              )}
            </div>

            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">{t('labUploadCT.observations', 'Observations')}</label>
              <textarea
                rows={4}
                value={observations}
                ...
                placeholder={t('labUploadCT.enterObservations', 'Enter observations...')}
              />
            </div>

            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">{t('labUploadCT.uploadCTScan', 'Upload CT Scan')}</label>
              <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-emerald-500 transition-colors">
                ...
                <p className="text-gray-600">{t('labUploadCT.uploadInstructions', 'Click or drag file here to upload')}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              ...
              >
                {t('labUploadCT.saveReport', 'Save Report')}
              </button>
            </div>
          </form>
        </div>
